  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 800, color: 'var(--text-heading)' }}>Guided Groups & Project Progress</h1>
            ...
            <Card key={group.id}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div className="grid-3" style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '12px' }}>
                    <div>Subject: <strong>{group.subjectName} ({group.subjectCode})</strong></div>
                    <div>Allocated Guide: <strong style={{ color: 'var(--text-heading)' }}>{group.guide}</strong></div>
                  ...
                  <div className="mobile-wrap" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ flex: 1, backgroundColor: 'var(--border-subtle)', height: '8px', borderRadius: '4px', overflow: 'hidden' }}>
                      <div style={{ width: `${group.overallProgress}%`, backgroundColor: group.overallProgress === 100 ? 'var(--badge-success-text)' : 'var(--text-heading)', height: '100%' }} />
                    </div>
                  ...
                  <tbody>
                    {tasks.length > 0 ? (
                      tasks.map(t => {
                        ...
                        return (
                          <tr key={t.task_id}>
                            <td data-label="Milestone" style={{ fontWeight: 700, color: 'var(--text-heading)' }}>{t.title}</td>
                            ...
                            <td data-label="File/Link" style={{ fontSize: '12px' }}>
                              {isDone ? (
                                ...
                                <span style={{ color: 'var(--text-disabled)' }}>Pending</span>
                              )}
                            </td>
                          </tr>
                        );
                      ...
                      <tr>
                        <td colSpan={3} style={{ textAlign: 'center', color: 'var(--text-disabled)' }}>No milestones published.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
                ...
                <table className="portal-table">
                  ...
                  <tbody>
                    {inspectingGroup.members.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan={3} style={{ textAlign: 'center', color: 'var(--text-disabled)' }}>No members enrolled.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
